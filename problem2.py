...
import Sample_sol_hw4_problem1 as sr  # sr = softmax regression 
#-------------------------------------------------------------------------
'''
    ...
    You could test the correctness of your code by typing `pytest -v test2.py` in the terminal.
    ...
    Notations:
            ...
            x: the feature vector of a data instance, a float numpy vector of shape (p, 1). 
            ...
            h: the number of outputs in the 1st layer (or the number of neuron in the first layer).
                ---------- 1st layer ----------------------
            W1: the weight matrix of the 1st layer, a float numpy array of shape (h, p). 
            b1: the bias values of the 1st layer, a float numpy vector of shape (h, 1).
                ---------- 2nd layer ----------------------
            W2: the weight matrix of the 2nd layer, a float numpy array of shape (h, c). Here c is the number of classes. 
            b2: the bias values of the 2nd layer, a float numpy vector of shape (c, 1).

            ---------- values ----------------------
                ---------- 1st layer ----------------------
            z1: the linear logits of the 1st layer, a float numpy vector of shape (h, 1). 
            a1: the sigmoid activations in the 1st layer, a float numpy vector of shape (h, 1). 
                The i-th element represents the sigmoid of the i-th logit z1[i].
                ---------- 2nd layer ----------------------
            z2: the linear logits, a float numpy vector of shape (c, 1). 
            a2: the softmax activations in the 2nd layer, a float numpy vector of shape (c, 1). 
            L: the multi-class cross entropy loss, a float scalar.

            ---------- partial gradients ----------------------
                ---------- 2nd layer ----------------------
            dL_da2: the partial gradients of the loss function L w.r.t. the activations a2, a float numpy vector of shape (c, 1). 
            ...
            da2_dz2: the partial gradient of the activations a2 w.r.t. the logits z2, a float numpy array of shape (c, c). 
                   The (i,j)-th element represents the partial gradient ( d_a2[i]  / d_z2[j] )
            dz2_dW2: the partial gradient of logits z2 w.r.t. the weight matrix W2, a float numpy array of shape (c, h). 
                   The (i,j)-th element represents the partial gradient of the i-th logit (z2[i]) w.r.t. the weight W2[i,j]:   d_z2[i] / d_W2[i,j]
            dz2_db2: the partial gradient of the logits z2 w.r.t. the biases b2, a float vector of shape (c, 1). 
                   Each element represents the partial gradient of the i-th logit z2[i] w.r.t. the i-th bias b2[i]:  d_z2[i] / d_b2[i]
            dz2_da1: the partial gradient of the logits z2 w.r.t. the inputs a1, a float numpy array of shape (c, h). 
                   The i-th element represents the partial gradient ( d_z2[i]  / d_a1[i] ). 
                ---------- 1st layer ----------------------
            da1_dz1: the partial gradient of the activations a1 w.r.t. the logits z1, a float numpy vector of shape (h, 1). 
                   The i-th element of da1_dz1 represents the partial gradient ( d_a1[i]  / d_z1[i] )
            dz1_dW1: the partial gradient of logits z1 w.r.t. the weight matrix W1, a float numpy array of shape (h, p). 
                   The (i,j)-th element represents the partial gradient of the i-th logit (z1[i]) w.r.t. the weight W1[i,j]:   d_z1[i] / d_W1[i,j]
            dz1_db1: the partial gradient of the logits z1 w.r.t. the biases b1, a float vector of shape (h, 1). 
            ...
            ---------- partial gradients of parameters ------------------
                ---------- 2nd layer ----------------------
            dL_dW2: the partial gradients of the loss function L w.r.t. the weights W1, a float numpy array of shape (h, p). 
                    The i,j-th element represents the partial gradient of the loss function L w.r.t. the i,j-th weight W1[i,j]:  d_L / d_W1[i,j].
            dL_db2: the partial gradients of the loss function L w.r.t. the biases b1, a float numpy vector of shape (c, 1).
                    The i-th element represents the partial gradient of the loss function L w.r.t. the i-th bias b1[i]:  d_L / d_b1[i].
                ---------- 1st layer ----------------------
            dL_dW1: the partial gradients of the loss function L w.r.t. the weights W1, a float numpy array of shape (h, p). 
                    The i,j-th element represents the partial gradient of the loss function L w.r.t. the i,j-th weight W1[i,j]:  d_L / d_W1[i,j].
            dL_db1: the partial gradients of the loss function L w.r.t. the biases b1, a float numpy vector of shape (h, 1).
...
def compute_z1(x,W1,b1):
    '''
        ...
        Input:
            x: the feature vector of a data instance, a float numpy vector of shape (p, 1). Here p is the number of features/dimensions.
            W1: the weight matrix of the first layer, a float numpy array of shape (h, p). Here h is the number of outputs in the first layer.
            b1: the bias values of the first layer, a float numpy vector of shape (h, 1).
        Output:
            z1: the linear logits, a float numpy vector of shape (h, 1). 
...
def compute_a1(z1):
    '''
        ...
        Input:
            z1: linear logits in the first layer, a float numpy vector of shape (h, 1). 
                Here h is the number of outputs in the 1st fully connected layer.
        Output:
            a1: the non-linear activations in the first layer, a float numpy vector of shape (h, 1). 
               The i-th element represents the sigmoid of the i-th logit z1[i].
...
def compute_z2(a1,W2,b2):
    '''
        ...
        Input:
            ...
            b2: the bias values of the 2nd layer, a float numpy vector of shape (c, 1).
        Output:
            z2: the linear logits of the 2nd layer, a float numpy vector of shape (c, 1). 
    '''
    #########################################
    ## INSERT YOUR CODE HERE
...
def compute_a2(z2):
    '''
        ...
        Input:
            z2: linear logits in the second layer, a float numpy vector of shape (c, 1). 
                Here c is the number of classes. 
        Output:
            a2: the non-linear activations in the 2nd layer, a float numpy vector of shape (c, 1). 
...
def compute_dL_da2(a2, y):
    '''
        ...
        Input:
            a2: the activations in the 2nd layer, a float numpy vector of shape (c, 1). Here c is the number of classes. 
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
        Output:
            dL_da2: the local gradients of the loss function L w.r.t. the activations a2, a float numpy vector of shape (c, 1). 
                    The i-th element dL_da2[i] represents the partial gradient of the loss function L w.r.t. the i-th activation a2[i]:  d_L / d_a2[i].
    '''
    #########################################
    ## INSERT YOUR CODE HERE
...
def compute_da2_dz2(a2):
    '''
        ...
        Input:
            a2: the activation values of softmax function, a numpy float vector of shape (c, 1). Here c is the number of classes.
        Output:
            da2_dz2: the local gradient of the activations a2 w.r.t. the logits z2, a float numpy array of shape (c, c). 
                   The (i,j)-th element represents the partial gradient ( d_a2[i]  / d_z2[j] )
    '''
    #########################################
    ## INSERT YOUR CODE HERE
...
def compute_dz2_dW2(a1,c):
    '''
        ...
        Input:
            a1: the activations of sigmoid function, a numpy float vector of shape (h, 1). 
        Output:
            dz2_dW2: the partial gradient of logits z2 w.r.t. the weight matrix W2, a float numpy array of shape (c, h). 
                   The (i,j)-th element represents the partial gradient of the i-th logit (z2[i]) w.r.t. the weight W2[i,j]:   d_z2[i] / d_W2[i,j]
    '''
...
def compute_dz2_da1(W2):
    '''
        ...
        Input:
            W2: the weights in the 2nd layer, a float numpy array of shape (c, h). 
        Output:
            dz2_da1: the local gradient of the logits z2 w.r.t. the inputs a1, a float numpy array of shape (c, h). 
                   The i-th element represents the partial gradient ( d_z2[i]  / d_a1[i] ). 
        Hint: you could solve this problem using 1 line of code.
    '''
    #########################################
...
def compute_da1_dz1(a1):
    '''
        ...
        Input:
            ...
            a1: the non-linear activations in the 1st layer.
        Output:
            da1_dz1: the local gradient of the activations a1 w.r.t. the logits z1, a float numpy vector of shape (h, 1). 
                   The i-th element of da1_dz1 represents the partial gradient ( d_a1[i]  / d_z1[i] )
        Hint: you could solve this problem using 1 line of code.
    '''
    #########################################
    ...
    '''
       Back Propagation: given an instance in the training data, compute the local gradients of the logits z, activations a, weights W and biases b in the two layers. 
        Input:
            ...
            y: the label of a training instance, an integer scalar value. The values can be 0,1,2, ..., or (c-1).
            a1: the activations of a training instance in the 1st layer, a float numpy vector of shape (h, 1).
            a2: the activations of a training instance in the 2nd layer, a float numpy vector of shape (c, 1).
        Output:
            dL_da2: the local gradients of the loss function w.r.t. the activations in the 2nd layer, a float numpy vector of shape (c, 1). 
            da2_dz2: the local gradient of the activation a2 w.r.t. the logits z2, a float numpy array of shape (c, c). 
            dz2_dW2: the partial gradient of logits z2 w.r.t. the weight matrix W2, a float numpy array of shape (c, h). 
            dz2_db2: the partial gradient of the logits z2 w.r.t. the biases b2, a float vector of shape (c, 1). 
...
def compute_dL_da1(dL_dz2,dz2_da1):
    '''
        ...
        Output:
            dL_da1: the partial gradient of the loss function w.r.t. the activations a1, a numpy float vector of shape (h, 1). 
...
def compute_dL_dz1(dL_da1,da1_dz1):
    '''
       ...
       (2 points)
        ...
        Output:
            dL_dz1: the partial gradient of the loss function w.r.t. the logits z1, a numpy float vector of shape (h, 1). 
...
def train(X, Y,h=3, n_layers=3, alpha=0.01, n_epoch=100):
    '''
       Given a training dataset, train the FC model by iteratively updating the weights W and biases b using the gradients computed over each data instance. 
        Input:
            ...
            Y: the labels of training instance, a numpy integer vector of shape (n, 1). The values can be 0 or 1.
    ...
    W1 = np.random.rand(h,p)
    b1 = np.random.rand(h,1)
    W2 = np.random.rand(c,h)
    b2= np.random.rand(c,1)
...
def check_da1_dz1(z1,delta= 1e-7):
    '''
        ...
        Input:
            z1: the input logits values of activation function, a float vector of shape (p, 1).
            delta: a small number for gradient check, a float scalar.
        Output:
            da1_dz1: the approximated local gradient of the activations a1 w.r.t. the logits z1, a float numpy vector of shape (p, 1). 
                   The i-th element of da1_dz1 represents the partial gradient ( d_a1[i]  / d_z1[i] )
    '''
    p = z1.shape[0]
    da1_dz1 = np.zeros((p,1))
    for i in range(p):
        d = np.zeros((p,1))
        d[i] = delta
