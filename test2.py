...
def test_compute_z1():
    ...
    assert z1.shape == (2, 1)
    assert np.allclose(z1, np.array([[0.4], [0.5]]), atol=1e-3) 
...
def test_compute_a1():
    ...
    a1 = compute_a1(np.array([1000., 100.]).reshape(-1, 1))
    assert np.allclose(a1.T, [1., 1.], atol=1e-2)

    a = compute_a1(np.array([-1000., -10.]).reshape(-1, 1))
...
def test_compute_z2():
    ...
    W2 = np.array([[0.5,-0.6,0.3],
                  [0.6,-0.5,0.2]])
    b2 = np.array([0.2, 0.3]).reshape(-1, 1)
...
def test_compute_da2_dz2():
    ...
    a = np.array([0.3, 0.7]).reshape(-1, 1)
    ...
    a = np.array([0.1, 0.2, 0.7]).reshape(-1, 1)
    da_dz = compute_da2_dz2(a)
...
def test_compute_da1_dz1():
    ...
    a1 = np.array([.5, .5, .3, .6]).reshape(-1, 1)
...
def test_compute_dL_da1():
    ...
    dL_dz2 = np.array([ 0.09554921, 0.14753129, 0.47769828,-0.72077878]).reshape(-1, 1)
    ...
    dL_da1_true = np.array([-0.13505987,-0.07568605, 0.28386814]).reshape(-1, 1)
...
def test_compute_dL_dz1():
    ...
    dL_da1  = np.array([-0.03777044, 0.29040313, -0.42821076, -0.28597724]).reshape(-1, 1)
    da1_dz1 = np.array([0.03766515, 0.09406613, 0.06316817, 0.05718137]).reshape(-1, 1)
    ...
    dL_dz1_true = np.array([-0.00142263, 0.0273171, -0.02704929, -0.01635257]).reshape(-1, 1)
...
def test_check_compute_gradients():
    ...
    for _ in range(20):
        ...
        x = np.array(10*np.random.random((p,1))-5)
        y = np.random.randint(c) 
        W1 = np.array(2*np.random.random((h,p))-1)
        b1 = np.array(np.random.random((h,1)))
        W2 = np.array(2*np.random.random((c,h))-1)
        b2 = np.array(np.random.random((c,1)))
        z1, a1, z2, a2 = forward(x, W1, b1, W2, b2)
        dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1= backward(x,y,a1,a2, W2) 
