...
def test_compute_dL_da():
    ...
    assert np.allclose(dL_da, np.array([0.,-2.]).reshape((-1, 1)), atol= 1e-3)

    a  = np.array([0.5,0.5])
    y = 0
    dL_da = compute_dL_da(a,y)
    assert np.allclose(dL_da, np.array([-2.,0.]).reshape((-1, 1)), atol= 1e-3)

    a  = np.array([0.1,0.6,0.1,0.2])
    y = 3
    dL_da = compute_dL_da(a,y)
    assert np.allclose(dL_da, np.array([0.,0.,0.,-5.]).reshape((-1, 1)), atol= 1e-3)
...
def test_check_dL_da():
    ...
    for _ in range(20):
        ...
        a = np.random.random((c,1)) # activation
        a = (a + 1)/(a+1).sum()
